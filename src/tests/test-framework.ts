...
import { TestCase, TestResult, DatabaseSnapshot } from '../core/types';
...
export class TestFramework {
  ...
  async runTests(testFile?: string, verbose: boolean = false): Promise<{
    total: number;
    passed: number;
    failedCount: number;
    results: TestResult[];
    passedTests: TestResult[];
    failed: Array<TestResult & { error: string }>;
  }> {
    const testFiles = testFile ? [testFile] : await this.findTestFiles();
    const allResults: TestResult[] = [];
    const passedTests: TestResult[] = [];
    const failed: Array<TestResult & { error: string }> = [];
  ...
  private async runSingleTest(testCase: TestCase): Promise<TestResult> {
    try {
      ...
      const runner = new FunctionRunner(initialSnapshot);
      
      const result = await runner.runFunction(
        this.createMockFunction(testCase.functionName, testCase.functionType),
        testCase.args,
        testCase.functionType
      );
      ...
      if (testCase.expectedState !== undefined) {
        const expectedDiff = this.stateDiffEngine.compareSnapshots(
          initialSnapshot,
  ...
  private createMockFunction(functionName: string, functionType: string) {
    return async (args: any, ctx: any) => {
      switch (functionName) {
        ...
        case 'getUser':
          const user = await ctx.db.get('users').filter((user: any) => user._id === args.userId).first();
          return user;
        case 'createUser':
  ...
  private getTestError(result: TestResult, testCase: TestCase): string {
    if (result.error) {
    ...
    if (testCase.expectedState !== undefined) {
      const expectedDiff = this.stateDiffEngine.compareSnapshots(
        testCase.initialSnapshot as DatabaseSnapshot,
        testCase.expectedState
      );
  ...
  async createTestFile(filePath: string, testCases: TestCase[]): Promise<void> {
    await fs.writeJSON(filePath, testCases, { spaces: 2 });
  }

  async validateTestFile(filePath: string): Promise<{
    valid: boolean;
    errors: string[];
  }> {
    const errors: string[] = [];
    ...
    try {
      ...
      for (const testCase of testCases) {
        ...
        if (!testCase.functionName) errors.push('Missing function name');
        if (!testCase.functionType) errors.push('Missing function type');
        if (!['query', 'mutation', 'action'].includes(testCase.functionType)) {
          errors.push('Invalid function type');
        }
        if (testCase.args === undefined) errors.push('Missing args (use {} if no args)');
      }
