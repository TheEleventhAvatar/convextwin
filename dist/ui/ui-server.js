...
const workflow_runner_1 = require("../workflows/workflow-runner");
const replay_validator_1 = require("../replay/replay-validator");
class TwinUIServer {
    constructor(options = {}) {
        this.server = null;
        ...
        this.eventTracker = new state_event_tracker_1.StateEventTracker(this.eventStore, options.snapshotName ?? 'default');
        this.workflowRunner = new workflow_runner_1.WorkflowRunner({
    ...
    async handleRequest(req, res) {
        ...
        if (req.method === 'POST' && workflowMatch) {
            const scenarioName = decodeURIComponent(workflowMatch[1]);
        ...
        if (req.method === 'POST' && pathname === '/api/replay/validate') {
            ...
            const validation = await (0, replay_validator_1.validateReplay)(snapshotName, events, this.currentSnapshot, './snapshots');
        ...
        if (tableMatch) {
            ...
            if (req.method === 'POST') {
                ...
                const db = new mock_database_1.MockDatabaseImpl(this.currentSnapshot.tables);
    ...
    groupWorkflowSessions(events) {
        ...
        return [...sessions.entries()].map(([workflowSessionId, sessionEvents]) => ({
            workflowSessionId,
            retryChainIds: [...new Set(sessionEvents.map(event => event.retryChainId).filter((value) => Boolean(value)))],
...
exports.TwinUIServer = TwinUIServer;
