...
class StateDiffEngine {
    ...
    generateHumanReadableDiff(diff) {
        const lines = [];
    ...
    applyDiff(baseSnapshot, diff) {
        ...
        for (const [tableName, updates] of Object.entries(diff.updated)) {
            if (!newTables[tableName])
                continue;
            for (const update of updates) {
                const recordIndex = newTables[tableName].findIndex((record) => record._id === update._id);
                if (recordIndex !== -1) {
                    for (const change of update.changes) {
        ...
        for (const [tableName, records] of Object.entries(diff.deleted)) {
            ...
            for (const recordToDelete of records) {
                const recordIndex = newTables[tableName].findIndex((record) => record._id === recordToDelete._id);
                if (recordIndex !== -1) {
